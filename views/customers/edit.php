...
<form method="post" action="<?= $cfg['APP_URL'] ?>/customers/edit?id=<?= $customer['id'] ?>">
  ...
  <label>الاسم</label>
  <input class="input" name="name" value="<?= Utils::h($customer['name']) ?>" required>

  <label>الهاتف</label>
  <input class="input" name="phone" value="<?= Utils::h($customer['phone']) ?>" required>

  <label>المنطقة</label>
  <input class="input" name="area" value="<?= Utils::h($customer['area']) ?>">
